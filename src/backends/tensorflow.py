    ...
    def _run_tf(self, config: BenchmarkConfig, is_reference: bool) -> Tuple[Benchmark, np.ndarray]:
        ...
        with tf.device(get_tf_device(config.device)):
            ...
            if not is_reference:
                ...
                # Run Profiling
                pf_options = tf.profiler.experimental.ProfilerOptions(host_tracer_level = 3, python_tracer_level = 1, device_tracer_level = 1)
                tf.profiler.experimental.start(PROF_PATH, options = pf_options)
                model_f(inputs)
